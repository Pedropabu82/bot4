...
class LiveMAStrategy:
    ...
    def log_trade(self,symbol,trade_type,entry,exit_price,result,timeframe):
        ...
        try:
            with open(os.path.join(DATA_DIR, "trade_log.csv"),'a') as f: f.write(','.join(map(str,row))+'\n')
        except: pass

    def get_recent_trades(self,symbol=None):
        try:
            df=pd.read_csv(os.path.join(DATA_DIR, "trade_log.csv")); df['timestamp']=pd.to_datetime(df['timestamp'])
